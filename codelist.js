...
program
    ...
    .action(({ logger, args, options }) => {
        ...
        in_excel_we_trust.forEach(xcl => {
            ...
            //check if folder exists and start with a clean one

            if (fs.existsSync(`${path_to_folder}${name_version}`)) {
                // cleanup
                fs.rmSync(`${path_to_folder}${name_version}`, { recursive: true, force: true }, (err) => {
                    log(err)
            ...
            JSON2file('.\\ESPD\\codelists\\external_code_list.json', extclJSON)
...
function process_code_lists(sph) {
    ...
    //create the folder
    //check if folder exists and start with a clean one
    if (fs.existsSync(`${path_to_folder}${name_version}\\${json_structure.name}`)) {
        // cleanup
        fs.rmSync(`${path_to_folder}${name_version}\\${json_structure.name}`, { recursive: true, force: true }, (err) => {
            log(err)
            fs.mkdirSync(`${path_to_folder}${name_version}\\${json_structure.name}`, { recursive: true }, (err) => {
                log(err)
            })
        })
    } else {
